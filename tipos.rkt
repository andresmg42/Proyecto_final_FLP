...
(define proc-list
 ...
 (define type-of-fusion-lang
   (lambda (result-texps names exps p-body tenv)
     (let*(
          (p-list (proc-list exps result-texps names (list) (list) (list) (list) (list) (list) (list)))
          ...
          (proc-names (cadddr p-list))

          (proc-results-types (expand-type-expressions (car (cddddr p-list))))
          
          (noproc-results-types (expand-type-expressions (caddr (cddddr p-list))))

          (noproc-names (cadr (cddddr p-list)))
          
          (arg-types (map (lambda (texps) (expand-type-expressions texps)) texpss)) 
                                                
          (result-types (expand-type-expressions result-texps))

          (the-proc-types (map proc-type arg-types proc-results-types))

          (tenv-for-proc (extend-tenv proc-names the-proc-types tenv))

          (tenv-for-body (extend-tenv noproc-names noproc-results-types tenv-for-proc))
       ...
       (for-each
        (lambda (ids arg-types body result-type)
          (if (proc? body)
              ...
              (check-equal-type!
               (type-of-expression body tenv-for-body) result-type body)
              
              ))
        idss arg-types exps result-types)
       ...
       (type-of-expression p-body tenv-for-body)
       )))
