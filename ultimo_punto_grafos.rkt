...
(define grammar-simple-interpreter
  '(
    ...
    (id-exp ("const" (arbno type-exp) identifier) const-exp)
    (id-exp ("var" (arbno type-exp) identifier) identifier-var-exp)
    (id-exp ("proc" (arbno type-exp) identifier) proc-id-exp)
     ...
     (let*(
       ...
       (for-each
        (lambda (pos body id)
          ...
          (let*(
              (list-name-texp (cases id-exp id (const-exp (texp name) (list name (car texp)))
                                (identifier-var-exp (texp name) (list name (car texp)))
                                (proc-id-exp (texp name) (list name (car texp)))))
      ...
      (env (extend-tenv-recursively id-exps exps tenv))
        ...
        (lambda (body id)
          (let*(
               ...
               (texp (cases id-exp id (const-exp (texp name) (car texp)) (identifier-var-exp (texp name) (car texp)) (proc-id-exp (texp name) (car texp))))
      ...
      (closure (ids body env)
           (cases expression body
             ;(eval-expression body (extend-env ids args (make-boolean-list (length args) #f) env))
                 (set-exp (id rhs-exp)
                          (eval-expression body (extend-env (list (cases expression rhs-exp (var-exp (id) id) (else 'error))) args (list #f) env)))
...
proc @inicialize=function ( String @m  String @mo)
{
BLOCK{
set @marca=@m
...
proc  @getMarca=function(){@marca}

proc  @getModelo=function(){@modelo}

proc  @setMarca=function(String @mr)
...
proc @setModelo=function(String @mod){
...
proc @send=function(String @mensaje String @mark String @model)

{

if {@mensaje == "inicialize"} then @inicialize 
else
if {@mensaje == "getMarca"} then app(@getMarca)
else
if {@mensaje == "getModelo"} then app(@getModelo)
else
...
var @objeto= app(@vehiculo)
}
{
BLOCK{
print(app(@objeto "getMarca"))
print(app(app(@objeto "inicialize") "chebrolet" "corsa"))
print(app(@objeto "getMarca"))
print(app(@objeto "getModelo"))
print(app(app(@objeto "setMarca") "renol"))
print(app(@objeto "getMarca"))
}}}
...
proc @send=function(String @mensaje)
...
proc @inicializeM=function(String @cil String @ma String @ml)
...
set @veh= app(@vehiculo)

app(app(@veh "inicialize") @ma @ml)
...
proc @getCilindrada=function(){@cilindrada}

proc @setClilindrada=function(String @c){set @cilindrada = @c}

proc @sendM=function(String @message)

{

if {@message == "inicialize"} then @inicializeM 
else
if {@message == "getCilindrada"} then app(@getCilindrada)
else
if{@message == "setCilindrada"} then @setCilindrada
else
if {@message == "getMarca"} then app(@veh "getMarca")
else
if {@message == "getModelo"} then app(@veh "getModelo")
else
if{@message == "setMarca"} then app(@veh "setMarca")
else
if{@message == "setModelo"} then app(@veh "setModelo")
else
"message_not_supported"
...
{
BLOCK{
app(app(@objeto "inicialize") "cien" "Honda" "cb")
app(app(@objeto "setMarca") "zuzuky")
app(@objeto "getMarca")
}}}
...
proc @setCilindrada=function(int @c){set @cilindrada = @c}
...
proc @inicializeM=function(int @cil String @ma String @ml)
...
set @veh= app(app(@vehiculo) @ma @ml)
...
var @moto1=app(app(@moto) 100 "honda" "cb")
var @moto2=app(app(@moto) 200 "Auteco" "pulsar")
var @moto3=app(app(@moto) 150 "yamaha" "rx")
}
{
BLOCK{


print("atributos_iniciales_objeto1")

print("atributos:")

print("Cilindrada:")
print(app(@moto1 "getCilindrada"))
print("Marca:")
print(app(@moto1 "getMarca"))
print("Modelo:")
print(app(@moto1 "getModelo"))

print("seteando_atributos::::::::::::::::::")

app(app(@moto1 "setCilindrada") 300)
app(app(@moto1 "setMarca") "Dukaty")
app(app(@moto1 "setModelo") "apache")
