     ...
     (let*(
       ...
       (for-each
        (lambda (pos body id)
          ...
          (let*(
            ...
            (cond
              ...
              (let(
                   ...
                   (body-type (expand-type-expressions body))
                   
                   )
                (vector-set! vec pos (proc-type arg-types body-type)))))]
          ...
          (cases expression body (function-exp (texps ids body-exp)
                                               
              (let*(
        
                   (arg-types (expand-type-expressions texps))
                   (body-type (type-of-expression body-exp (extend-tenv ids arg-types tenv)))
                   
                   )
                (vector-set! vec pos (proc-type  arg-types body-type))))
            
            (app-exp (rand rands)
                     (vector-set! vec pos (type-of-expression rand tenv)))
            
              (else (vector-set! vec pos result-type))) ]
      ...
      (env (extend-tenv-recursively id-exps exps tenv))
        ...
        (lambda (body id)
          (let*(
               ...
               (texp (cases id-exp id (const-exp (texp name) texp) (identifier-var-exp (texp name) texp)))
             ...
             (if (equal? result-type function-type) 0
    ...
    (cases expression exp
      ...
      (if-exp (test-exp true-exp false-exp)
              (let ((test-type (type-of-expression test-exp tenv))
                ...
                true-type))
      (function-exp (texps ids body) (type-of-proc-exp texps ids body tenv))
...
(define extend-env-recursively2
  (lambda (id-exps bodies old-env)
    (let* (
      (len (length id-exps))
      (vec (make-vector len))
      (names (map (lambda (id) (cases id-exp id (const-exp (texp name) name) (identifier-var-exp (texp name) name))) id-exps))
...
var proc @len= function(list<int> @l int @count)
{
...
var proc  @factorial = function(int @x)
...
var proc @inicialize=function (String @marca String @modelo)
{
BLOCK{
set @marca=@marca
...
var proc @getMarca=function(){@marca}

var proc @getModelo=function(){@modelo}

var proc @setMarca=function(String @marca)
...
var proc @setModelo=function(String @modelo){
...
var proc @send=function(String @mensaje)
