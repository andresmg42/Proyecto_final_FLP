  ...
  (lambda (env sym)
    (cases environment env
      ...
      (extended-env-record (syms vals env)
                           (let* (
                                 (list-ids (map (lambda (id)  (cases id-exp id (const-exp (texp id) id) (identifier-var-exp (texp id) id) (proc-id-exp (texp id) id))) syms))
                                 (pos (rib-find-position sym list-ids))
                                 (id (list-ref syms pos))
                                 (const (cases id-exp id (const-exp (texp id) #t) (identifier-var-exp (texp id) #f) (proc-id-exp (texp id) #f)))
                                 )
                             (if (number? pos)
